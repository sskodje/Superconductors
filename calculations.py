...
def calculate_delta_pluss_minus(delta, diag_coupling, off_diag_coupling, epsilon_pluss, epsilon_minus):
    ...
    """ 
    delta_pluss = np.abs(delta[0])
    delta_minus = np.abs(delta[1])
...
def anneal_delta_pluss_minus(diag_coupling, off_diag_coupling, epsilon_pluss, epsilon_minus):
    ...
    """    
    parameter_space_min = [1e-7] * 2
    parameter_space_max = [1e-2] * 2
...
def calculate_initial_values(delta_i, delta_f, gamma, kx, ky, N,t, spinorbit, SoC_quench):
    """
    ...
    delta_i = (delta_i_pluss + delta_i_minus)/2
    ...
    temp = lambda_i * np.sum(F_0_pluss) + lambda_i_od *np.sum(F_0_minus)
    ...
    arguments = (lambda_i,lambda_f, epsilon_solve_pluss_init, epsilon_solve_minus_init, epsilon_solve_pluss_quench, epsilon_solve_minus_quench, gamma_pluss, gamma_minus)  ## *0.7test dual band SC, change lambda ratio lambda_f, lambda_i *0.6 etc
...
def calculate_function_array(y,t,lambda_i,lambda_f, epsilon_solve_pluss_init, epsilon_solve_minus_init, epsilon_solve_pluss_quench, epsilon_solve_minus_quench, gamma_pluss, gamma_minus):
    if t==0:
        coupling = lambda_i
